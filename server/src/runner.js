  ...
  async function worker() {
    while (index < bales.length) {
      ...
      try {
        ...
        if (!info.valid) {
          errors++;
          await setBale('error', 0, JSON.stringify(read.json ?? read.text), 'No data / transient error', label, bale.id);
          ...
          errors++;
          await setBale('error', 0, JSON.stringify(info.raw), 'Device has no check point — not registered', label, bale.id);
          emit(jobId, { type: 'bale', id: bale.id, status: 'error', label, message: 'device not registered' });
        } else if (info.alreadyDone) {
          already++;
          await setBale('already', 1, JSON.stringify(info.raw), null, label, bale.id);
          emit(jobId, { type: 'bale', id: bale.id, status: 'already', label, checkPoint: info.checkPoint });
        } else if (mode === 'check') {
          await setBale('checked', 0, JSON.stringify(info.raw), null, label, bale.id);
          ...
          if (after.alreadyDone) {
            confirmed++;
            await setBale('confirmed', 1, JSON.stringify(res.json ?? res.text), null, confirmLabel, bale.id);
            ...
            await setBale('error', 0, JSON.stringify(res.json ?? res.text), detail, confirmLabel, bale.id);
            emit(jobId, { type: 'bale', id: bale.id, status: 'error', label: confirmLabel, message: detail });
          }
        }
      } catch (err) {
        errors++;
        await setBale('error', 0, null, String(err.message || err), null, bale.id);
